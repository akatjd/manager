            ...
            var valueProcessor = {
                "formatter": function formatter() {
                    ...
                    if (U.isFunction(_col.formatter)) {
						...
						// 민성 sparkLineDataTemp 및 sparkline 관련 데이터
						if(that.key == 'sparkLineDataTemp'){
							mapList.push(that.item.sparkLineDataMapList);
							kmsMachineIdAry.push(that.item.machineId);
							kmsDataAry.push(that.item.sparkLineData);
						}else if(that.key == 'arrDistributionCnt'){ // 공구 수명에서는 machineId + toolName 으로 id 지정
							kmsMachineIdAry.push(that.item.machineId + that.item.toolName);
							kmsDataAry.push(that.item.distributionMapList);
						}else if(that.key == 'ctDistribution'){
							kmsMachineIdAry.push(that.item.machineId);
							kmsDataAry.push(that.item.distributionMapList);
						}else if(that.key == 'ctDistributionDetail'){
							kmsMachineIdAry.push(that.item.machineId + that.item.toolName);
							kmsDataAry.push(that.item.distributionMapList);
						}else if(that.key == 'distributionCt'){
							kmsMachineIdAry.push(that.item.machineId + "" + that.item.productId);
							kmsDataAry.push(that.item.distributionMapList);
						}else if(that.key == 'distributionDetailCt'){
							kmsMachineIdAry.push(that.item.machineId + "" + that.item.productId + that.item.sectionNumber + that.item.toolName);
							kmsDataAry.push(that.item.distributionMapList);
						}else if(that.key == 'changedTimeArr'){
							hcMachineIdAry.push(that.item.machineId);
							hcDataAry.push(that.item.changedTimeArr);
						}else if(that.key == 'machineHoldTime'){
							kmsMachineIdAry.push(that.item.machineId);
							kmsDataAry.push(that.item.machineHoldTime);
						}
...
//			console.log(this);
			...
			// 민성 sparkline 추가
			if(this.config.name == 'toolLifeListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
				        series: [{
				            data: kmsDataAry[i],
				            pointStart: 1,
							name: 'count',
						...
						tooltip: {
                			pointFormatter: function () { return '<b>' + (this.y).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + ' (개)</b>';  },
			...
			}else if(this.config.name == 'temperatureListGrid'){
				
				for(var i=0; i<kmsMachineIdAry.length; i++){
					var arrDate = new Array();
					var sumDataArr = new Array();
					for(var j=0; j<mapList[i].length; j++){
						arrDate.push(mapList[i][j].x);
					}
					for(var k=0; k<arrDate.length; k++){
						var tempArr = new Array();
						tempArr.push(arrDate[k]);
						tempArr.push(kmsDataAry[i][k]);
						sumDataArr.push(tempArr);
					}
					var arrTemp = new Array(kmsDataAry[i].length);
					arrTemp.fill(100);
					var rangeValue = 100;
					var maxValue = Math.max(...kmsDataAry[i]);
					if(rangeValue < maxValue) rangeValue = maxValue;
					...
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
						...
						tooltip: {
                			pointFormatter: function () { return '<b>' + (this.y).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + ' ℃</b>';  },
							backgroundColor: "rgba(255,255,255,255)",
							outside: false,
							borderWidth: 1,
						},
			...
			}else if(this.config.name == 'toolChangeTimeListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
				        series: [{
				            data: kmsDataAry[i],
			...
			}else if(this.config.name == 'toolChangeTimeDetailListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
				        series: [{
				            data: kmsDataAry[i],
			...
			}else if(this.config.name == 'ctListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
				        series: [{
				            data: kmsDataAry[i],
			...
			}else if(this.config.name == 'ctDetailListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
				        series: [{
				            data: kmsDataAry[i],
			...
			}else if(this.config.name == 'alarmListGrid'){
				for(var i=0; i<kmsMachineIdAry.length; i++){
//					Highcharts.chart(kmsMachineIdAry[i], {
//					    chart: {
//					        type: 'bar',
//					        margin: [-20, 0, -20, 0],
...
//					    exporting: { enabled: false }
//					});
					var seriesData = '00:00:00';
					if(kmsDataAry[i] !== null){
						var hour = kmsDataAry[i]/3600 < 10 ? '0'+kmsDataAry[i]/3600 : kmsDataAry[i]/3600;
						var min = (kmsDataAry[i]%3600)/60 < 10 ? '0'+(kmsDataAry[i]%3600)/60 : (kmsDataAry[i]%3600)/60;
						var sec = kmsDataAry[i]%60 < 10 ? '0'+kmsDataAry[i]%60 : kmsDataAry[i]%60;
						seriesData = hour + ':' + min + ':' + sec;
					}
					$("#"+ kmsMachineIdAry[i]).highcharts('SparkLine', {
					    chart: {
					        type: 'bar',
        ...
        var repaintHeader = function repaintHeader(_elTarget, _colGroup, _bodyRow) {
            ...
            for (var tri = 0, trl = _bodyRow.rows.length; tri < trl; tri++) {
                ...
                for (var ci = 0, cl = _bodyRow.rows[tri].cols.length; ci < cl; ci++) {
                    ...
                    var _rowspan = col.rowspan;
                    if(cfg.header.rowspan){
                    	_rowspan = cfg.header.rowspan;
                    	if(col.rowIndex == 1){continue;};
                    }
                    ...
                    }(), 'data-ax5grid-column-colindex="' + col.colIndex + '" ', 'data-ax5grid-column-rowindex="' + col.rowIndex + '" ', 'colspan="' + col.colspan + '" ', 'rowspan="' + _rowspan + '" ', 'class="' + function (_col) {
                        var tdCSS_class = "";
